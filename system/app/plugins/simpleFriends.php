<?php
	...
	function friendList()
	{
		...
		if($infoRelationsNum == '0'){
			...
			<div class="friend_head" style="background: url(https://www.habbo.com/habbo-imaging/avatarimage?figure='. filter($infoFriends['look']) .'&head_direction=2&action=wav&headonly=1)">
			</div>
			</div>
			</td>
			<td>
		...
		if($infoRelationsNum == '0'){
			...
			Je hebt geen vrienden in deze categorie!
			<img src="'.$config['hotelUrl'].'/templates/brain/style/images/icons/iconbest.png" class="friend_icon" style="margin-top: -3px;float: right;">
			</div>
			';
			}else{
			if($infoRelationsNum == '1'){
				...
				$infoRelationsNum = $infoRelationsNum - 1;
				$infoNumtext = "You have <b>" . $infoRelationsNum . "</b> friends more in the category";
			}
			echo '
			<div class="friend_2"">
			<table>
		...
		if($infoRelationsNum == '0'){
			...
			Je hebt geen vrienden in deze categorie!
			<img src="'.$config['hotelUrl'].'/templates/brain/style/images/icons/iconheat.png" class="friend_icon" style="margin-top: -3px;float: right;">
			</div>
			';
			}else{
			if($infoRelationsNum == '1'){
