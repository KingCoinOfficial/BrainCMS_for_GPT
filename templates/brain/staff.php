...
<?php
	...
	<div style="width: 600px;"class="columleft">
		...
		<?php
			$getRanks = $dbh->prepare("SELECT id,name,badgeid,tab_colour FROM ranks WHERE id in (9,8,7,6,5,4,3)  ORDER BY id DESC");
			$getRanks->execute();
			...
			<div class="' . $Ranks['tab_colour'] . ' title">' . $Ranks['name'] . '</div> 
				...
				if ($getMembers->RowCount() > 0)
				{
					while ($member = $getMembers->fetch())
					{
						...
						<div id="column" style="border: 2px dotted rgba(0, 0, 0, 0.2);margin-top: -10px;margin-left: 10px;margin-right: 10px;margin-bottom: 10px;float: left;height:55px;width: 55px;border-radius: 555px;-moz-border-radius: 555px;-webkit-border-radius: 555px;background:url('.$config['habboImagingUrl'].''.$look.'&head_direction=2&direction=2&action=wlk&gesture=sml) no-repeat;background-position: 50% 10%;"></div>
