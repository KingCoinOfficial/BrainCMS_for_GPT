...
<?php
	...
	<div style="width: 500px; margin-left: 0px;" class="columright">
				...
				<div id="column" style="    width: 460px;height: 400px;background: url('/templates/brain/style/images/icons/Hotel_HP_BG.png') no-repeat;background-color: #FFF;">
					<div class='boxbg1'>
						<div class="platte" style="float: left;margin-left:-5px;margin-top:5px;width:119px;height:165px">
							<img src="<?= $config['habboImagingUrl'] ?><?= userHome('look'); ?>&direction=2&head_direction=3&action=wlk&gesture=sml" style="-webkit-filter: drop-shadow(0 1px 0 #FFFFFF) drop-shadow(0 -1px 0 #FFFFFF) drop-shadow(1px 0 0 #FFFFFF) drop-shadow(-1px 0 0 #FFFFFF);margin-top: -20px;position: absolute;margin-left: 25px;">
						</div>
						<div style="color:black" class="mission">	 <?= filter(userHome('motto')); ?></div>
					</div>
					<div class="boxbg3">
					...
					<div class="boxbg2">
						<div class="sternbg">
							<div style="color:black" class="smalltext" style="text-align: center;"></div>
							<div style="color:black" class="smalltext" style="width: 100%;text-align: center;"><b><?= userHome('username'); ?>:</b> <?= $lang["Hhometext"] ?></div>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="box">
			<div class="title blue"><?= $lang["Hrelationsshipof"] ?> <?= userHome('username'); ?></div>
			<div class="mainBox" style="float;left">
	...
	<div style="width: 470px;margin-left: 10px;" class="columleft">
			...
			<div class="mainBox" style="float;left">
				<?php
					...
					else
					{
						...
						echo userHome('username').' '.$lang["Hnobadges"].'';
					}
				?>
			</div>
		</div>
		<div class="box">
			...
			<div class="mainBox" style="float;left">
				<div style="width: 450px; min-height: 529px; overflow-y: scroll;">
					<?php
						$userId = userHome('id');
						...
						if (!$sql->RowCount() == 0)
						{
							while($news = $sql->fetch())
							{
								...
								echo'
								<a href="/home/'.filter($getUserData['username']).'"> <img style="float: right;" src="'.$config['habboImagingUrl'].''.filter($getUserData['look']).'&direction=3&head_direction=3&action=wav&gesture=sml&size=s&headonly=0"> 
						...
						{
							echo userHome('username').' '.$lang["Hnofriends"].'';
						}
					?>
				</div>
			</div>
