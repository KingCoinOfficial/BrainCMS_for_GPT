...
<?php
	...
	<div style="margin-left: 0px;" class="columright">
		...
		<div class='box'>
			...
			<div class='mainBox'>
				<?= newsLike() ?>
				<b style="font-size:15px; margin-top: 4px; "><?= newsLikeCount() ?> <?= $lang["Nuserslikenews"] ?></b> <img style="float:right;" src="/templates/brain/style/images/account/image_969.png">
				
			</div>
			<form method="post">
					<input type="submit" class="buttonlike" value="<?= $lang["Nuserslikenewsbutton"] ?>" name="likenews">
				</form>
		</div>
	</div>
				...
				<?php
					...
					if ($getMessage->RowCount() > 0)
					{
						while ($getMessageData = $getMessage->fetch())
						{
							...
							<div id="newscolumn" style="border: 2px dotted rgba(0, 0, 0, 0.2);padding: 10px;margin-top: 10px;margin-left: 10px;margin-right: 10px;margin-bottom: 10px;float: left;height:55px;width: 55px;border-radius: 555px;-moz-border-radius: 555px;-webkit-border-radius: 555px;background:url( '.$config['habboImagingUrl'].''.filter($user["look"]).'&amp;head_direction=3&amp;action=wav) no-repeat;background-position: 50% 10%;"></div>
							';
		...
		<div class='box'>
			...
			<div class='mainBox'>
				...
				<form method="post">
					...
					<input type="submit" class="button" value="<?= $lang["Ncommandbutton"] ?>" name="newscomment" style="margin-top: 10px; margin-bottom: 10px;">
				</form>
			</div>
		</div>
